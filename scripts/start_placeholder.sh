...
ffmpeg -f lavfi -i "color=c=0x1a1a2e:s=${WIDTH}x${HEIGHT}:r=${FPS}" \
    ...
    -vf "drawtext=text='Stream Starting Soon':fontsize=120:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2-60,drawtext=text='%{localtime\:%I\\\\\:%M\\\\\:%S %p PT}':fontsize=80:fontcolor=white:x=(w-text_w)/2:y=(h-text_h)/2+80" \
